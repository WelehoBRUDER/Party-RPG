...
function calculateDamage(dmg, def, res) {
  return Math.floor(((dmg * dmg / (def + dmg)) * (1 - res)) * random(1.15, 0.9));
}
...
function _attack(actor, target) {
  ...
  combat.characters[target.battleIndex].damage(dmg);
...
function showTargets(id) {
  ...
  else if (power.targets == "self") { power.action(actor); return; }
  const button = document.createElement("button");
  button.addEventListener("click", mainButtons);
  button.textContent = "Cancel";
  combat.buttons.append(button);
...
const actor_combat_text = (actor) => {
  return `<i>${actor.img}[actor_combat ${actor.enemy == true ? "enemy" : "ally"}]<i> <c>${actor.color}<c>${actor.name}<c>black<c>`
}
...
function combatSort() {
  ...
  for(let i = 0; i < combat.characters.length; i++) {
    combat.characters[i].battleIndex = i;
  }
}

function teamLowest(char) {
  let lowest = 100;
  let lowestChar;
  combat.characters.forEach(chr=>{
    if((char.enemy && chr.enemy) || (!char.enemy && !chr.enemy)) {
      if(lowest > chr.hpRemaining()) { lowest = chr.hpRemaining(); lowestChar = chr; }
    }
  });
  return {l: lowest, c: lowestChar}
}